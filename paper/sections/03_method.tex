...
difficulty--uncertainty reward, and
\(\widetilde a_i^{(t)}\in[0,1]\) is the candidate's normalized
projection onto the current representation anchor. Given budget \(B\le N\),
...
trusted reference response, and (iii) is cheap enough to run once
over the full pool and cache. The construction below meets these
...
rather than an empty or generic one, effects common to any
condition---response fluency, length, boilerplate---cancel in the
ratio. This matched negative control distinguishes the contrast
...
requires labels, judges, or selector training. The gate decides
...
scale---which we now check on a realized trajectory.
...
comparison is the trajectory-aware Davis--Kahan envelope reported in
Appendix~\ref{app:anchor-stability-diagnostics}.
...
zero-anchored: \(G_i=0\) annihilates the score, and the selection
key in Eq.~\eqref{eq:tag-selection-key} orders every positive-gate
candidate ahead of the zero block, so the veto is exact and
non-compensatory---no value of the dynamic terms can re-admit a
vetoed pair while admissible candidates remain. Additive or
...
representation-anchor modulation and yields the gated carrier
\(G_i\mathcal R_i^{(t)}\), the CO baseline of
