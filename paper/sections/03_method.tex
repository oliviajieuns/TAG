...
\(w^{(t)}\) for their variance-ratio coefficient. Superscript \(t\)
denotes a quantity refreshed under \(\theta_{t-1}\); gate quantities
are computed once at \(\theta_0\) and carry no refresh superscript.
...
\end{equation}
Here \(\sigma(z)=1/(1+e^{-z})\) is the logistic sigmoid,
\(s_G\) is fixed from the same reference distribution, and
...
\(\theta_{t-1}\). Following dynamic difficulty--uncertainty
selection~\cite{kung2023activeit,dataagent}, we define the carrier
reward
\begin{equation}
  \mathcal{R}_i^{(t)}
  =
...
task-family groups \(\{\mathcal D_g\}_{g=1}^{J}\), and let
...
sequence \(x\) at layer \(l\) and token position \(k\), where
\(k=1,\dots,K_x\) indexes the \(K_x\) token positions of \(x\) and
\(l=1,\dots,L\) the retained layers. We define the
...
information. Figure~\ref{fig:anchor} illustrates the one-layer
geometry: probe displacements, the oriented anchor \(v_l^{(t)}\),
and a candidate's projection contribution. The raw scores are
min--max normalized over the candidate pool. Let
...
\begin{equation}
  d_l^{(t)}
  =
  \min_{\varsigma\in\{\pm1\}}
  \left\|
  \tilde v_l^{(t)}
  -
  \varsigma \tilde v_l^{(t-1)}
  \right\|_2
  =
  \sqrt{
  2
...
\begin{table}[t]
\centering
\caption{Anchor-displacement diagnostic for \textsc{TAG} and the
matched Carrier-Only control CO~\((\lambda{=}0)\). The invariant
statistic is \(d_l^{(t)}\in[0,\sqrt{2}]\). Med. and peak are
transition-level layer summaries aggregated over post-warmup
refreshes; worst step is the maximum retained layer--refresh pair.
Status: PASS if no retained pair exceeds \(d_l^{(t)}=1\)
(sign-calibrated angle \(<60^\circ\)); see text.}
...
Carrier-Only CO~($\lambda{=}0$) control;
...
theory~\cite{kittler1998combining,genest1986combining}---and for
\(G_i>0\) the score is, in log space, simply the additive pool
\(\log s_i^{(t)}=\log G_i+\log\mathcal R_i^{(t)}
+\log\bigl(1+\lambda\,\widetilde a_i^{(t)}\bigr)\), with
\(G_i=0\) playing the role of the pool's \(-\infty\) veto.
...
of \(1{+}\lambda\) and can neither veto nor dominate; as noted in
Section~\ref{sec:reliability-gate}, only these two dynamic views
recur per refresh. Finally, each view is independently ablatable:
