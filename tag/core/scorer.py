"""TAG scoring helpers (paper §3.3, Algorithm 1).

Pure-function utilities for the deterministic TAG selection pipeline. No
...
Pipeline composition (one refresh step t):
    ...
    s_i                   = legacy_score(R, widetilde-align, λ)  (paper Eq. 10)
...
def calibrated_utility(
    ...
    NOT part of the paper-faithful TAG pipeline. Paper §3.3 final
    ...
    directly to ``legacy_score`` (paper Eq. 10).

    Kept here so ablation runs can opt-in to z-score variant by manually
    composing ``legacy_score(calibrated_utility(R), widetilde_align, lam)``
    ...
    *,
    ...
    Args:
        ...
        collapse_eps: if max-min < collapse_eps the pool is degenerate
            ...
            ``1 + λ·0.5 = const`` would silently turn the anchor-boosted score into the
...
def legacy_score(
    ...
    Args:
        R: (N,) composite reward (paper Eq. 3). Raw — NOT z-scored.
            ...
            ``legacy_score(calibrated_utility(R), widetilde_align, lam)``
    ...
    if R.shape != alignment_norm.shape:
        raise ValueError(
            f"legacy_score: shape mismatch R={tuple(R.shape)} "
            f"vs widetilde-align={tuple(alignment_norm.shape)}"
        )
    if lam < 0:
        raise ValueError(f"legacy_score: lam must be >= 0, got {lam}")
...
#                           base checkpoint; tag/core/reliability.py)
    ...
    *,
    ...
    Args:
        reliability: (N,) Q_i in [0, 1] (calibrated counterfactual
            fidelity; see ``tag.core.reliability``).
        ...
        lam: anchor weight λ ≥ 0 (same role as in :func:`legacy_score`).
