...
paper / prior-work (DOTS, Active IT) convention compatibility. The TAG
...
def collect_episode(
    ...
    *,
    ...
    gate: s_i = G_i · R_i · (1 + λ·widetilde-align_i). Expected keys:
        ...
        gate          (N,) G in [0,1] from ``tag.core.gate.compute_gate``,
    ...
    logger.info(
        ...
        "mvf" if mvf is not None else ("tag" if tag is not None else "legacy"),
    ...
    if apply_anchor:
        ...
        if _alignment_collapsed:
            logger.error(
                "Trajectory-anchor alignment COLLAPSED (max-min < 1e-8) | "
    ...
    if mvf is None:
        ...
        # rule, so raw R goes into `legacy_score` directly.
        if apply_anchor:
            score = legacy_score(R, alignment, lam)
            boost = 1.0 + lam * alignment
            logger.info(
                "Trajectory-anchored score | epoch=%d | lam=%.3f | R_mean=%.4f | "
