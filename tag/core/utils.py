...
def clear_runtime_caches() -> None:
    ...
    1. **Python GC arena** — `gc.collect()`. A stale `tag.train` worker
    ...
    3. **HF datasets ``Dataset.map`` fingerprint cache** — opt-in via
       ``TAG_FRESH_DATA_CACHE=1``. The legacy default is to reuse the
       ...
       and re-tokenises from scratch (see tag/data/alpaca.py).
    ...
    fresh_data = os.environ.get("TAG_FRESH_DATA_CACHE", "0") == "1"
    logger.info(
...
# Project root = parent of the `tag/` package directory.
# utils.py lives at <root>/tag/core/utils.py, so go up three levels.
...
def _collect_path_candidates(ref: str, anchor: Optional[Path] = None) -> List[Path]:
    ...
    Order:
      ...
      3. The tag/ package root (so users can `python -m tag.train` from
