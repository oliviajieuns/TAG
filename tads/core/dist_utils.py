"""Distributed utilities for TADS selection.
...
def log_rank_predicates(extra: Optional[Callable[[], bool]] = None) -> None:
    ...
    print(
        f"[tads.dist] node={node} rank={rank} local_rank={local} "
...
def all_gather_concat(local_tensor: torch.Tensor) -> torch.Tensor:
    ...
    is sharded across DDP ranks (the common case for the TADS Step 1 forward).
    """
