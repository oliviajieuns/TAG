...
def completeness_from_dataset(
    ...
    1. ``text_complete`` — raw-text heuristic computed at tokenisation time
       (``tads.data.sft_prompts.text_is_complete``): terminal punctuation /
        ...
        if text_flag is None:
            if not warned_missing_text:
                logger.warning(
                    ...
                    "TADS_FRESH_DATA_CACHE=1). Falling back to the token-level "
    ...
    if scale is None:
        ...
        if pos.numel() == 0:
            ...
            logger.warning(
                ...
                "(tads.mvf.reliability_scale / reliability_ref_file).", scale,
            )
    if scale <= 0:
