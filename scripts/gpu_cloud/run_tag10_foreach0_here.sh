...
# Run one Table-2 TAG seed on the current allocated node.  The host name
# selects seed/world/batch; all three nodes keep effective global batch 128.
# AdamW foreach is disabled explicitly to remove its full-parameter-size
# temporary tensor-list peak on 80GB GPUs.
...
preflight() {
  resolve_node
  cd "$REPO"
  [[ "$(git rev-parse HEAD)" == "$PIN" ]] || die "repo HEAD is not hotfix commit $PIN"
  ...
  [[ "${#visible_gpus[@]}" -eq "$NPROC" ]] || \
    die "$HOST sees ${#visible_gpus[@]} GPU(s), expected $NPROC"
  mapfile -t free_mib < <(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits | tr -d ' ')
  for mib in "${free_mib[@]}"; do
    [[ "$mib" -ge 78000 ]] || die "a GPU has only ${mib} MiB free; every GPU must be idle"
  done
  [[ ! -e "$RUN_DIR" ]] || die "hotfix run already exists: $RUN_DIR (use --status; do not overwrite)"
...
assert cfg["use_8bit_optimizer"] is False
...
worker() {
  ...
  echo "TRAIN_START host=$HOST seed=$SEED nproc=$NPROC batch=$BATCH grad_accum=$GRAD_ACCUM effective_batch=$((NPROC * BATCH * GRAD_ACCUM)) adamw_foreach=false"
